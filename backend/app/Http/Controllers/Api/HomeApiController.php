...
class HomeApiController extends Controller
{
    /**
     * GET /api/home
     * Same data HomeController@index passes to Inertia, as plain JSON for
     * the standalone React Router frontend.
     */
    public function index(): JsonResponse
    {
        $localities = Locality::query()
            ...
            ->orderBy('name')
            ->get(['id', 'name', 'slug', 'base_price_multiplier']);

        $services = Service::query()
            ->where('is_active', true)
            ->orderBy('display_order')
            ->get(['id', 'slug', 'name', 'short_description', 'icon_name', 'display_order']);
        ...
        $testimonials = Testimonial::query()
            ->with(['project:id,title,slug,locality,hero_image_path'])
            ->where('is_featured', true)
            ->latest()
            ->limit(5)
            ->get(['id', 'customer_photo', 'customer_name', 'content', 'rating', 'project_id'])
            ->map(fn ($testimonial) => [
                'id' => $testimonial->id,
